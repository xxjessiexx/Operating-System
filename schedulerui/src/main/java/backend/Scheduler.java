...
public class Scheduler {
    ...
    Deque<backend.Process> readyQueue = new ArrayDeque<>();
    Deque<backend.Process> blockedQueue = new ArrayDeque<>();
    Deque<backend.Process> PQ0 = new ArrayDeque<>(); // highest priority queue for processes that have been in the ready state
    // the longest --MLFQ
    Deque<backend.Process> PQ1 = new ArrayDeque<>();
    Deque<backend.Process> PQ2 = new ArrayDeque<>();
    Deque<backend.Process> PQ3 = new ArrayDeque<>(); // lowest priority queue for processes that have been in the ready state
    // the shortest -- MLFQ
    int usedTime = 0;
    backend.Process HRRNprocess = null;
    backend.Process MLFQprocess = null;
    backend.Process RRprocess = null;
    private int quantum;
    ...
    public backend.Process getRRProcess() {
        return RRprocess;
    }
    ...
    public double calculateResponseRatio(backend.Process process, int globalTime) {
    ...
    public void addProcess(backend.Process process, int globalTime) {
    ...
    public void addUnblockedProcess(backend.Process process, int globalTime) { // same implementation as addProcess for now
        process.pcb.processState = ProcessState.READY;
    ...
    public void removeTerminatedProcess(backend.Process process) {
    ...
    public void removeBlockedProcess(backend.Process process) {
    ...
    public backend.Process roundRobin(int timeQuantum, int globalTime) {
    ...
    public backend.Process HRRN(int globalTime) {   //check lw fe process and running and not finished return it else lw finished set hrrnprocess=null to fund a new
        ...
        if (HRRNprocess == null) {
            ...
            for (backend.Process process : readyQueue) { // get the HRRN ratio for each process still in the ready queue
    ...
    public backend.Process MultilevelFeedbackQueue(int globalTime) { // chooses according to the highest priority non-empty
    ...
    public void updateMLFQ(backend.Process process, int globalTime) { // called right after a process is done executing to set
    ...
    public backend.Process SchedulingAlgorithm(String algorithm, int globalTime) {
        printQueues();

        switch (algorithm) {
            case "RoundRobin":
                return roundRobin(quantum, globalTime);
    ...
    public String readyQueueAsString() {
        return readyQueue.toString();
    }

    public String blockedQueueAsString() {
        return blockedQueue.toString();
    }

    public String pq0AsString() {
        return PQ0.toString();
    }

    public String pq1AsString() {
        return PQ1.toString();
    }

    public String pq2AsString() {
        return PQ2.toString();
    }

    public String pq3AsString() {
        return PQ3.toString();
    }
